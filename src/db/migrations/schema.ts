import { pgTable, integer, text, unique, boolean, foreignKey, jsonb, pgPolicy, check, uuid, timestamp, primaryKey, bigint, pgEnum } from "drizzle-orm/pg-core"
...
export const guildQuests = pgTable("guild_quests", {
	...
	pgPolicy("Guild quests are deletable by authenticated users.", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.role() = 'authenticated'::text)` }),
	pgPolicy("Guild quests are updatable by authenticated users.", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Guild quests are insertable by authenticated users.", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Guild quests are viewable by authenticated users.", { as: "permissive", for: "select", to: ["public"] }),
